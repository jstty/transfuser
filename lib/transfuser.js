...
ConfigManager.prototype._resolvePath = function(file){
    ...
    // Relative to Root (windows, e.g. C:/path/to/file)
    else if(process.platform === 'win32' && /^[A-Za-z]:.+/.test(file)) {
        // root path so just use it
        pathFile = file;
    }
    // Relative to Root (unix, e.g. /path/to/file)
    else if(process.platform !== 'win32' && file.charAt(0) == "/") {
        // root path so just use it
        pathFile = file;
    }
